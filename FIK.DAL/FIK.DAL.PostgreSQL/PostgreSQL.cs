...
namespace FIK.DAL.PostgreSQL
{
    public class PostgreSQL
    {
        ...
        /// <summary>
        /// Traditional sql query for data insert update delete. where parameter not implemented. so 
        /// sql injection is possible. transaction is implemented
        /// </summary>
        /// <param name="SQL">List of sql string </param>
        /// <param name="ErrorMsg">if any error occurred then provide the output </param>
        /// <returns></returns>
        public bool ExecuteQuery(List<string> SQL, ref string ErrorMsg)
        {
            ErrorMsg = "";
            bool result = false;
            NpgsqlTransaction oTransaction = null;
            NpgsqlCommand oCmd = null;
            ...
            try
            {
                if (connection != null)
                {
                    ...
                    {
                        errorQuery = s;
                        oCmd = new NpgsqlCommand(s, connection);
        ...
        /// <summary>
        /// Generate Insert query based on Model passed as parameter
        ...
        public bool Insert<T>(object dataObject, string specificProperty, string ExlcudeAutogeneratePrimaryKey, string customTable, ref string ErrorMsg)
        {
            ...
            try
            {
                if (connection != null)
                {
                    ...
                    {
                        oCmd = new NpgsqlCommand(dynamicQuery, connection);
                        oCmd.CommandTimeout = 0;
        ...
        /// <summary>
        /// Generate update query based on Model passed as parameter, where clause parameter data not used for update
        /// using SqlCommand Parameter add, sql injection not possible
        /// </summary>
        /// <param name="dataObject"> pass a single object or list of object </param>
        ...
        public bool Update<T>(object dataObject, string specificProperty, string WhereClasseParameter, string customTable, ref string ErrorMsg)
        {
            ...
            for (int i = 0; i < props.Count; i++)
            {
                ...
                // record set update only which is not available in where clause
                //if (!WhereClasseParameter.ToUpper().Contains(prop.Name.ToUpper()))
                if (!isExist(WhereArray, prop.Name))
                {
                    if (!string.IsNullOrEmpty(specificProperty))
        ...
        /// <summary>
        /// Generate Insert or update query based on CompositeModel passed as parameter
        /// using SqlCommand Parameter add, sql injection not possible
        /// </summary>
                ...
                if (connection != null)
                {
                    ...
                    foreach (CompositeModel c in dataObject.GetRecordSet())
                    {
                       ...
                       TypeDescriptor.GetProperties(c.ObjectType);
                        ...
                        {
                            oCmd = new NpgsqlCommand(sqlList[index], connection);
                            oCmd.CommandTimeout = 0;
        ...
        /// <param name="msg"> if any error occurred then provide the output </param>
        ...
        /// <param name="prefix"> Serial Prefix </param>
